...
class TestConfiguration:
    ...
    async def test_delete_parent_category(self, client):
        ...
        with patch.object(connect, 'get_storage_async', return_value=storage_client_mock):
            with patch.object(c_mgr, 'delete_parent_category', return_value=async_mock()) as patch_delete_parent_cat:
                ...
                r = await resp.text()
                json_response = json.loads(r)
                assert {'message': 'Parent category-south is deleted'} == json_response
    ...
    async def test_delete_category(self, client):
        ...
        c_mgr = ConfigurationManager(storage_client_mock)
        with patch.object(connect, 'get_storage_async', return_value=storage_client_mock):
            with patch.object(c_mgr, 'delete_category', return_value=async_mock()) as patch_delete_parent_cat:
                resp = await client.delete('/foglamp/category/{}'.format("south"))
                assert 200 == resp.status
                r = await resp.text()
                json_response = json.loads(r)
                assert {'message': 'Category south and its children deleted'} == json_response
            patch_delete_parent_cat.assert_called_once_with('south')
